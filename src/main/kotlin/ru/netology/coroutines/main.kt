...
fun main() {
    val posts = mutableListOf<PostAdv>()

    with(CoroutineScope(EmptyCoroutineContext)) {
        launch {
            try {
                val _posts = getPosts(client)
                    .map { post ->
...
suspend fun getCommentWithAuthor(id: Long): List<CommentAdv> {
    val com = mutableListOf<CommentAdv>()
    getComments(client, id).map {
       com.add(CommentAdv(it.id, getAuthor(client, it.authorId), it.content, it.published, it.likedByMe, it.likes))
    }
    return com
}
